...
impl Monkey {
    ...
    fn modify_item(&mut self, index: usize, new_value: i32) {
    ...
    fn do_operation(&mut self, index: usize) {
        ...
        let val: i32 = match val_str {
            "old" => self.items[index],
            _ => val_str.parse::<i32>().unwrap(),
        };
        match op {
...
impl Test {
    ...
    fn execute_test(&self, worry_lvl: i32) -> usize {
    ...
    monkey
        ...
        .for_each(|(i, line)| {
            match i {
                ...
                1 => m.items = line.replace("  Starting items: ", "").split(", ").map(|num| num.parse::<i32>().unwrap_or(0)).collect(),
                2 => m.op_str = line.replace("  Operation: ", ""),
                3 => m.test.divisible_by = line.replace("  Test: divisible by ", "").parse::<i32>().unwrap_or(0),
